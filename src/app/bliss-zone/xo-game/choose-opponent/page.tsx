...
import { ArrowLeft, Loader2, Users, CheckSquare, X } from "lucide-react";
...
export default function XOChooseOpponentPage() {
    ...
    const handleRequestGame = async (opponentId: string) => {
        ...
        if (error) {
            console.error("Error creating XO game:", error);
            toast({ variant: 'destructive', title: 'Could not start game', description: error.message });
        } else if (data) {
            router.push(`/bliss-zone/xo-game/requesting/${data.id}`);
        }
    };
        ...
        <div className="flex h-dvh flex-col bg-background text-foreground">
            <header className="flex h-16 flex-shrink-0 items-center border-b px-4 relative">
                ...
                <div className="flex items-center gap-3 mx-auto">
                     <div className="h-6 w-6 flex items-center justify-center gap-0.5"><X className="h-5 w-5" /><CheckSquare className="h-5 w-5" /></div>
