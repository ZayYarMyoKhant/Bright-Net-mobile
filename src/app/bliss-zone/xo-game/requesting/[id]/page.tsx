...
import type { XOGame } from '@/lib/data';
...
export default function XORequestingPage({ params: paramsPromise }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchGameData = async () => {
            ...
            if (error || !data) {
                ...
                router.push('/bliss-zone/xo-game/choose-opponent');
                return;
            }
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel(`xo-game-request-${gameId}`)
            .on<XOGame>(
                'postgres_changes',
                ...
                (payload) => {
                    ...
                    if (game.status === 'accepted') {
                        router.push(`/bliss-zone/xo-game/play/${gameId}`);
    ...
    return (
        <div className="flex h-dvh flex-col items-center justify-center bg-background text-foreground text-center p-4">
            <div className="h-16 w-16 flex items-center justify-center gap-0.5"><X className="h-10 w-10 text-primary" /><CheckSquare className="h-10 w-10 text-primary" /></div>
